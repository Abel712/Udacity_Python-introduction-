...
def getMovieUrl(category, location):
	#ps:诚然该Task只是一个拼接字符串的过程，最简单的过程为url = url_prefix + category + "," + location，我考虑安排一个判断在爬虫环节前验证爬虫目标合理，因为不合理的目标还不如不开展行动。
...
def task4(category,location):
	...
	soup = BeautifulSoup(expanddouban.getHtml(getMovieUrl(category, location),True), "lxml")
	...
	for i in range(1,cache_jump_number+1):
		class_cache = Movie(cache_name_dict[i],cache_rate_dict[i],location.replace(' ',''),category.replace(' ',''),cache_info_link_dict[i],cache_cover_link_dict[i])
...
def task5(category):
	...
	return print("反馈：已将爬取内容输出到movies.csv文件中。")
...
def task6(input_csv='movies.csv'):
	...
	return print("反馈：已将统计结果输出到output.txt文件中。")
